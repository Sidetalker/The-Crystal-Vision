...
class OllamaClient:
    ...
    def __init__(
        ...
        self._embed_ok: Optional[bool] = None  # None untested, then sticky
...
def user_facing_ollama_error(exc: BaseException, model: str) -> str:
    ...
    if isinstance(exc, requests.exceptions.ConnectionError):
        return (
            "[I can't reach my local model — is Ollama running? "
            f"Try: ollama serve, then ollama pull {model}]"
        )
    if isinstance(exc, requests.exceptions.Timeout):
        return (
            "[That took too long — the model may still be loading. "
