    ...
    def stream_chat(self, messages: list[dict[str, Any]],
        ...
        for line in response.iter_lines(decode_unicode=True):
            ...
            if chunk.get("error"):
                ...
                raise requests.exceptions.HTTPError(str(msg), response=response)
...
def user_facing_spacexai_error(exc: BaseException, model: str) -> str:
    ...
    if isinstance(exc, RuntimeError) and "XAI_API_KEY" in str(exc):
        return ("[SpaceXAI needs an API key. Set XAI_API_KEY "
                "(https://console.x.ai), then try again.]")
    if isinstance(exc, RuntimeError) and "SpaceXAI" in str(exc):
        return f"[{exc}]"
    if isinstance(exc, requests.exceptions.ConnectionError):
        return (f"[I can't reach SpaceXAI (api.x.ai) — check your network and "
                f"try again. Model was {model}.]")
    if isinstance(exc, requests.exceptions.Timeout):
        return ("[That took too long on SpaceXAI. Reasoning models can need a "
                "few minutes — try again, or lower temperature.]")
    if isinstance(exc, requests.exceptions.HTTPError):
        status = getattr(exc.response, "status_code", None)
        if status in (401, 403):
            return ("[SpaceXAI rejected the API key. Check XAI_API_KEY at "
                    "https://console.x.ai.]")
        if status == 429:
